...
public class CubeChaserState extends AbstractAppState {
    ...
    private float rotateVelocity = 15;
    ...
    public void update(float tpf) {
        ...
        if (results.size() > 0) {
            ...
            if (target.getControl(CubeChaseControl.class) != null) {
                if (cam.getLocation().distance(target.getLocalTranslation()) < 10) {
                    target.move(cam.getDirection());
                    System.out.println(target.getControl(CubeChaseControl.class).Hello()
                    + " and I am running away from " + cam.getLocation());
                    counter++;
                } else {
                    target.rotate(0, rotateVelocity * tpf, 0);
                }
            }
